...
mysqli_stmt_bind_param($cekBentrok, "isss", $id_lab, $tanggal_pinjam, $jam_selesai, $jam_mulai);
mysqli_stmt_execute($cekBentrok);

if (mysqli_num_rows(mysqli_stmt_get_result($cekBentrok)) > 0) {
    header("Location: dashboard.php?error=Jadwal+bentrok!+Lab+sudah+digunakan+pada+jam+tersebut");
    exit;
}
